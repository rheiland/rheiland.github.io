/*=========================================================================
  ...
  Module:    $RCSfile: vtkPBufferRenderWindow.cxx,v $
  Language:  C++
...
=========================================================================*/
// .NAME vtkPBufferRenderWindow - Render OpenGL scene offscreen.
// .SECTION Description
...
//
// .SECTION see also
//  vtkOpenGLOffscreenRenderWindow, vtkOpenGLRenderWindow, vtkWin32OffscreenRenderWindow

#include <stdlib.h>
#include <math.h>
#include <iostream.h>
#include "vtkPBufferRenderWindow.h"
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::WriteImage(char *filename)
{
  //XImage *img = this->GetImage(); 
  vtkDebugMacro(<< " vtkPBufferRenderWindow::WriteImage\n");  
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::WritePPMPbuffer(char *filename,
					 ...
					 unsigned int *img)
{
  ...
  if (!(ppmFile = fopen(filename, "w"))){
    offErrorMacro(<< "vtkPBufferRenderWindow::WritePPMPbuffer Could not open file " << filename << "\n" );  
    return;
  }
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::WritePPMXImage(char *filename,XImage *img)
{
  FILE *ppmFile;
  register int x,y;
  register short bindex;
...
//----------------------------------------------------------------
int vtkPBufferRenderWindow::WriteJPEGFile(char *filename, int quality)
{
  vtkDebugMacro(<< " vtkPBufferRenderWindow::WriteJPEGFile\n");  
...
//----------------------------------------------------------------
unsigned char* vtkPBufferRenderWindow::WriteJPEGMem(int quality, int *size)
{
  unsigned char *jpegImgOut;

  vtkDebugMacro(<< " vtkPBufferRenderWindow::WriteJPEGMem\n");  
  int width = this->Size[0];
  int height = this->Size[1];
  offDebugMacro(<< "\tSize is " << width << ':' << height << "\n" );  
  unsigned char *rgbBuffer = this->GetPixelData(0,0,width-1,height-1,1);
...
//----------------------------------------------------------------
XVisualInfo *vtkPBufferRenderWindow::GetDesiredVisualInfo()
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetDesiredVisualInfo\n");
...
//----------------------------------------------------------------
// Constructor
vtkPBufferRenderWindow::vtkPBufferRenderWindow()
{
#ifdef USE_PBUFFER
  ...
  vtkDebugMacro(<< "vtkPBufferRenderWindow::vtkPBufferRenderWindow\n");
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::Destroy()
{
  if (this->ContextId) {
    glXDestroyContext( this->DisplayId, this->ContextId);
#ifdef USE_PBUFFER
...
void vtkPBufferRenderWindow::Start(void)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::Start\n");
...
void vtkPBufferRenderWindow::Frame(void)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::Frame\n");

  glFlush();
  if (!this->AbortRender && this->DoubleBuffer&&this->SwapBuffers)
    {

      vtkDebugMacro(<< "vtkPBufferRenderWindow::Frame swapbuffers\n");
...
void vtkPBufferRenderWindow::StereoUpdate(void)
{
...
void vtkPBufferRenderWindow::WindowConfigure()
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::WindowConfigure\n");

  // this is all handled by the desiredVisualInfo method
}

//----------------------------------------------------------------
void vtkPBufferRenderWindow::QueryPbuffer(Display *dpy,GLXPbufferSGIX pbuf)
{
  unsigned int value;
  /* GLX_WIDTH_SGIX, GLX_HEIGHT_SGIX, GLX_LARGEST_PBUFFER_SGIX */
  offDebugMacro(<< "vtkPBufferRenderWindow.cxx : QueryPbuffer() glXQueryGLXPbufferSGIX() Info\n");
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::QueryConfig(Display *dpy, GLXFBConfigSGIX config)
{
  int value;
...
void vtkPBufferRenderWindow::WindowInitialize (void)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::WindowInitialize\n");
...
//			   None};
  ...
  if(!config) vtkErrorMacro(<< "vtkPBufferRenderWindow::WindowInitialize : no matching config found\n");
  ...
  for(confignum=0;confignum<nattribs;confignum++){
    ...
    offDebugMacro(<< "\tRGBAdepth=" << r << ":" << g << ":" << b << ":" << a << ":" << d << "\n");
    if(r==8 && g==8 && b==8 && a==8 && d > 16) break;
  }
  offDebugMacro(<< "\tconfignum(after 8)=" << confignum << "\n");

  if(confignum>=nattribs){
    /* scan for a 12 bit config */
    vtkDebugMacro(<< "Attempt for config failed.  Try 12 bit\n");
    for(confignum=0;confignum<nattribs;confignum++){
      int r,g,b,a,d;
      glXGetFBConfigAttribSGIX(this->DisplayId,config[confignum],GLX_RED_SIZE,&r);
      glXGetFBConfigAttribSGIX(this->DisplayId,config[confignum],GLX_GREEN_SIZE,&g);
  ...
  offDebugMacro(<< "\tconfignum(after 12)=" << confignum << "\n");
  ...
  offDebugMacro(<< "\tconfignum(after 10)=" << confignum << "\n");


  this->ContextId = glXCreateContextWithConfigSGIX(this->DisplayId, 
                           config[confignum], 
...
void vtkPBufferRenderWindow::Initialize (void)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::Initialize\n");
...
void vtkPBufferRenderWindow::SetFullScreen(int arg)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetFullScreen\n");
...
void vtkPBufferRenderWindow::PrefFullScreen()
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::PrefFullScreen\n");
...
// Description:
// Resize the window.
void vtkPBufferRenderWindow::WindowRemap()
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::WindowRemap\n");
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::SetPosition(int x,int y)
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetPosition\n");
  this->Position[0]=x;
  this->Position[1]=y;
...
void vtkPBufferRenderWindow::SetSize(int x,int y)
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetSize\n");
...
//----------------------------------------------------------------
int *vtkPBufferRenderWindow::GetSize()
{
  offDebugMacro(<< "vtkPBufferRenderWindow::GetSize() " << Size[0] << ":" << Size[1] << "\n");
  return this->Size;
}

//----------------------------------------------------------------
int vtkPBufferRenderWindow::GetDesiredDepth(){
  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetDesiredDepth\n");
  XVisualInfo *v;
...
Visual *vtkPBufferRenderWindow::GetDesiredVisual ()
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetDesiredVisual\n");
  XVisualInfo *v;
  // get the default visual to use 
  v = this->GetDesiredVisualInfo();
  return v->visual;  
...
Colormap vtkPBufferRenderWindow::GetDesiredColormap ()
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetDesiredColormap (JMS: This is *VERY* problematic for Pbuffers!!!!\n Find a way to avoid doing this\n");
  XVisualInfo *v;
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------
unsigned char *vtkPBufferRenderWindow::GetPixelData(int x1, int y1, int x2, int y2, 
						int front)
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetPixelData\n");
  long     xloop,yloop;
  int     y_low, y_hi;
  int     x_low, x_hi;
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::SetPixelData(int x1, int y1, int x2, int y2,
				      unsigned char *data, int front)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetPixelData\n");
...
//----------------------------------------------------------------
float *vtkPBufferRenderWindow::GetRGBAPixelData(int x1, int y1, int x2, int y2, int front)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetRGBAPixelData\n");
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::SetRGBAPixelData(int x1, int y1, int x2, int y2,
					  float *data, int front)
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetRGBAPixelData\n");
...
//----------------------------------------------------------------
float *vtkPBufferRenderWindow::GetZbufferData( int x1, int y1, int x2, int y2  )
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetZbufferData\n");
...
//----------------------------------------------------------------
void vtkPBufferRenderWindow::SetZbufferData( int x1, int y1, int x2, int y2,
					 float *buffer )
{

  vtkDebugMacro(<< "vtkPBufferRenderWindow::SetZbufferData\n");
...
XImage *vtkPBufferRenderWindow::GetImage()
{
  vtkDebugMacro(<< "vtkPBufferRenderWindow::GetImage\n");
...
//----------------------------------------------------------------
#ifdef notdef
void vtkPBufferRenderWindow::MakeCurrent()
{
  Bool retval;

  vtkDebugMacro(<< "vtkPBufferRenderWindow::MakeCurrent\n");
  ...
  if (!retval) {
    cout<<"vtkPBufferRenderWindow::MakeCurrent: glXMakeCurrent is FALSE"<<endl;
    cout<<"  attempting to release..."<<endl;
    retval = glXMakeCurrent(this->DisplayId,None,NULL);
    if (!retval) 
      cout<<" release failed"<<endl;
    else
      cout<<" release successful"<<endl;

    cout<<"  attempting to re-do..."<<endl;
    retval = glXMakeCurrent(this->DisplayId,this->WindowId,this->ContextId);
    if (!retval) 
      cout<<" re-do failed"<<endl;
    else
      cout<<" re-do successful"<<endl;
  }
}
#endif
