...
# Create the "outer" circle of points
zDist = 184.0 + 40.0  # approx distance from center out to end of triangle
zDelta = 2*zDist / (numPts - 1)
c1 = array('f',[])    # we will store pts in an array
pts = vtkPoints()   # the 'pts' object will hold all our points (x,y,z)
theta = thetaStart
z = zDist
R = 280
...
# Create the "inner" circle of points
zDist = 92.0 + 40.0  # approx distance from center out to top of triangle
zDelta = 2*zDist / (numPts - 1)
c2 = array('f',[])
theta = thetaStart
z = zDist
...
for i in range(0, numPts):  # explain 'range' (0,1,...,numPts-1)
    ...
    c2.append(y)
    c2.append(z)
    pts.InsertPoint(i+numPts, x, y, z)
    theta = theta + thetaDelta
    z = z - zDelta
...
numQuads = 1
numQuads = numPts - 1
for it in range(0,numQuads):
  quads.InsertNextCell(4)   # number of points on polygon
  quads.InsertCellPoint(ioff)
  quads.InsertCellPoint(ioff+1)
  quads.InsertCellPoint(ioff+numPts+1)
  quads.InsertCellPoint(ioff+numPts)
  ioff = ioff + 1
...
qpdm = vtkPolyDataMapper()
qpdm.SetInput(qpd)

qpda = vtkActor()
qpda.SetMapper(qpdm)
qpda.GetProperty().SetColor(1.0, 0.0, 0.0)  # set (red,green,blue) colors
